    ...
    * */
    laypage({
        ...
        ,jump: function(e, first){ //触发分页后的回调
            ...
            var url="/userAction.do?method=list";
            $.ajax({
                url: url,
                async: false,//同步调用
                data:{
...
function setData(data,page) {
            ...
            for(var i=0;i<list.length;i++){
                userInfoHtml = userInfoHtml+
                    ...
                    " <a  class=\"del_btn layui-btn layui-btn-small layui-btn-danger \"   user-id=\""+list[i].userId+"\" title=\"删除\" user-name=\""+list[i].userName+"\"><i class=\"layui-icon\"></i></a>\n" +
                    "  </td>" +
                    "</tr>";
            }
